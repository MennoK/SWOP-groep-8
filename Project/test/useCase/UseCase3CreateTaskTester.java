...
public class UseCase3CreateTaskTester {
	...
	@Test
	public void createTask() throws LoopingDependencyException, InvalidTimeException{
		//create a project
		...
		assertEquals(50, project1.getAllTasks().get(0).getAcceptableDeviation(),0.001);
		...
		project1.createTask("task with dependency", Duration.ofHours(20), 50,dependencyList);
	
		assertEquals(2, project1.getAllTasks().size());
		assertEquals(project1.getAllTasks().get(0), project1.getAllTasks().get(1).getDependencies().get(0));
		
		//create an alternative task for a failed task 
		//first we let the dependent task fail
		project1.getAllTasks().get(1).updateStatus(LocalDateTime.of(2015, 03, 07 ,02,00), LocalDateTime.of(2015, 03, 07 ,05,00), true);
		project1.createTask("alternative task", Duration.ofHours(20), 50,project1.getAllTasks().get(1));
		
		assertEquals(3, project1.getAllTasks().size());
		assertEquals("alternative task", project1.getAllTasks().get(2).getDescription());
	}
}

...
public class UseCase3CreateTaskTester extends UseCaseTestBasis {
	...
	public void createTask() {
		...
		// create a project
		Project project1 = tmc.createProject("Project 1", "Description 1", now
		...
		Task task2 = Task
				...
				.addDependencies(task1).build(project1);

		assertEquals(2, project1.getAllTasks().size());
		assertTrue(project1.getAllTasks().contains(task2));
		...
		Task task3 = Task.builder("alternative task", Duration.ofHours(20), 50)
				.setOriginalTask(task1).build(project1);

		assertEquals(3, project1.getAllTasks().size());
		assertTrue(project1.getAllTasks().contains(task3));
		...
		Task task4 = Task.builder("desc", Duration.ofHours(2), 2)
				.addRequiredResourceType(resType, 1).build(project1);
		assertEquals(1, task4.getRequiredResourceTypes().size());
		assertTrue(project1.getAllTasks().contains(task4));
	}
}

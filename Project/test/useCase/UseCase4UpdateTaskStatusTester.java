...
public class UseCase4UpdateTaskStatusTester {
	...
	public void setUp() {
		...
		Task.builder("Task 1", Duration.ofHours(8), 0.4).build(project1);
		Task.builder("Task 2", Duration.ofHours(8), 0.4).build(project1);
		task1 = project1.getAllTasks().get(0);
		task2 = project1.getAllTasks().get(1);

		// task 3 has dependency on task2
		Task.builder("Task 3", Duration.ofHours(8), 0.4)
				.addDependencies(task2).build(project1);
		// task 4 had depndency on task 2
		Task.builder("Task 4", Duration.ofHours(8), 0.4)
				.addDependencies(task2).build(project1);

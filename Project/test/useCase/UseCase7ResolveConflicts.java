...
public class UseCase7ResolveConflicts {
	...
	public void setUp() {
		...
		project = tmc.getProjectExpert().createProject("proj", "proj description", now.plusYears(1));
		
		developer = tmc.getDeveloperExpert().createDeveloper("Jimmy Doge");
		
		plannedConflictingTask = Task.builder("Already Planned Task", Duration.ofHours(8), 0.2).build(project);
		
		originalTask = Task.builder("a task", Duration.ofHours(8), 0.2).build(project);
		
		Planning.builder(
				now, plannedConflictingTask,
				developer,
